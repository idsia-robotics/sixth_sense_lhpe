...
def get_body_joints_names_by_groups(group_names: str) -> List[str]:
    if group_names == "all":
        group_names = body_joints_groups_list

    return list(set([body_joints_groups[g] for g in group_names]))


def get_body_segments_joint_numbers() -> List[List[int]]:
    return [
        [0, 1, 2, 3],  # spine_coords
        [2, 4, 5, 6, 7, 8, 9],  # left_arm_coords
        [7, 10],  # left_hand_coords
        [2, 11, 12, 13, 14, 15, 16],  # right_arm_coords
        [14, 17],  # right_hand_coords
        [0, 18, 19, 20, 21],  # left_leg_coords
        [0, 22, 23, 24, 25],  # right_leg_coords
        [3, 26, 27],  # head_coords
        [29, 28, 27, 30, 31],  # face_coords
    ]
